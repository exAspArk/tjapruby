...
struct Statement *creat_return_stmt(struct Expressions_list *expr_list);

// бинарные операции
struct Expression * create_two_expr(enum ExprType Type,struct Expression *lhs,struct Expression *rhs);

// унарные операции
struct Expression * create_unar_op_expr(enum ExprType Type, struct Expression *rhs);
...
struct Expression * CreateExpr(enum ExprType exprType);
...
struct Expression * CreateCallArrayExpr(struct Expression *name,struct Expression *expr);
...
struct Expression * CreateArray(struct Expressions_list *expr_l);
